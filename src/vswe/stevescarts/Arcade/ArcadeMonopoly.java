...
public class ArcadeMonopoly extends ArcadeGame {
	...
	public void drawBackground(GuiMinecart gui, int x, int y) {
		...
		float smallgridX = x / SCALE - ((MinecartModular.MODULAR_SPACE_WIDTH / SCALE) - (PLACE_HEIGHT * 2 + (BOARD_WIDTH - 1) * PLACE_WIDTH)) / 2;
		float smallgridY = y / SCALE - ((MinecartModular.MODULAR_SPACE_HEIGHT / SCALE) - (PLACE_HEIGHT * 2 + (BOARD_HEIGHT - 1) * PLACE_WIDTH)) / 2;		

		boolean foundHover = false;
		
		if (selectedPlace != -1) {
			drawPropertyOnBoardWithPositionRotationAndScale(gui, places[selectedPlace], selectedPlace, true, false, (int)((MinecartModular.MODULAR_SPACE_WIDTH / 0.75 - ((getId(selectedPlace) == 0) ? PLACE_HEIGHT : PLACE_WIDTH)) / 2),  (int)((MinecartModular.MODULAR_SPACE_HEIGHT / 0.75 - PLACE_HEIGHT) / 2 ), 0, 0.75F);						
		} 
		...
		for (int i = 0; i < places.length; i++) {
			...
			if (!foundHover && getModule().inRect((int)smallgridX, (int)smallgridY, getSmallgridPlaceArea(i))) {
				if (selectedPlace == -1) {
					drawPropertyOnBoardWithPositionRotationAndScale(gui, places[i], i, true, false, (int)((MinecartModular.MODULAR_SPACE_WIDTH / 0.75 - ((getId(i) == 0) ? PLACE_HEIGHT : PLACE_WIDTH)) / 2),  (int)((MinecartModular.MODULAR_SPACE_HEIGHT / 0.75 - PLACE_HEIGHT) / 2 ), 0, 0.75F);
				}
	...
	private void drawPropertyOnBoard(GuiMinecart gui, Place place, int id, int side, int i, boolean hover) {
		...
		offX += ((int)(MinecartModular.MODULAR_SPACE_WIDTH / SCALE) - (PLACE_HEIGHT * 2 + (BOARD_WIDTH - 1) * PLACE_WIDTH)) / 2;
		offY += ((int)(MinecartModular.MODULAR_SPACE_HEIGHT / SCALE) - (PLACE_HEIGHT * 2 + (BOARD_HEIGHT - 1) * PLACE_WIDTH)) / 2;
	...
	public void mouseClicked(GuiMinecart gui, int x, int y, int b) {
		...
		if (currentCard != null && cardScale == 1F) {
			int[] rect = new int[] {(MinecartModular.MODULAR_SPACE_WIDTH - CARD_WIDTH) / 2, (MinecartModular.MODULAR_SPACE_HEIGHT - CARD_HEIGHT) / 2, CARD_WIDTH, CARD_HEIGHT};
