...
public abstract class ModuleBase {
	...
	/**
	 * If {@link isPlaceholder} returns true you can get the object controlling the simulation of the client only cart.
	...
	/**
	 * Returns the amount of stacks that this module can store. This will use {@link hasSlots}, {@link getInventoryWidth} and
	 * {@link getInventoryHeight} to calculate the size, this can however be overridden for more advanced usages.
	...
	/**
	 * The width of slots in the basic slot allocation. Used by the default {@link getInventorySize} to make standard
	...
	/**
	 * The height of slots in the basic slot allocation. Used by the default {@link getInventorySize} to make standard
	 * slot allocation easier
	...
	/**
	 * Get the list of slots used by this module. These have already been generated by {@link generateSlots}
	 ...
	 * just leave this and use {@link getSlot} instead (as well as setting {@link getInventoryWidth} and {@link getInventoryHeight})
	 ...
	 * function and return a new slots depending on where it's located. Shouldn't be used if you're overriding {@link generateSlots}
	 ...
	 * @param slot The slot start id, this is the local id for this module.
	 * @param slot The slot end id, this is the local id for this module.
	 * @param item The ItemStack to be set.
	 */
    ...
    /**
     * Whether the cart should allocate room for this interface. By default this also allocates slots, see {@link hasSlots}
	 ...
	 */	
	@SideOnly(Side.CLIENT)	 
	public void drawImage(GuiMinecart gui, Icon icon, int targetX, int targetY, int srcX, int srcY, int sizeX, int sizeY) {
		//create a rectangle and call the other drawImage function to do the job
	 ...
	 */
	@SideOnly(Side.CLIENT)
	public void drawImage(GuiMinecart gui, Icon icon, int[] rect, int srcX, int srcY) {
		//the rectangle need to be valid
		if (rect.length < 4) {
			return;
		}else{
	 ...
	 * change the srcY when drawing images for instance, see {@link drawImage}.
	 */
	public int handleScroll(int rect[]) {
		...
		}else if(y + rect[3] > getCart().MODULAR_SPACE_HEIGHT) {
			rect[3] = Math.max(0,getCart().MODULAR_SPACE_HEIGHT-y);
			return 0;
	...
	public final void readFromNBT(NBTTagCompound tagCompound, int id) {
		...
		if (getInventorySize() > 0) {
			NBTTagList items = tagCompound.getTagList(generateNBTName("Items",id));
	 ...
	 * @param info The information that should be sent as the first parameter to {@link updateGuiData}
	 */
	...
	protected FakePlayer getFakePlayer() {
		return FakePlayerFactory.getMinecraft(getCart().worldObj);
	}
