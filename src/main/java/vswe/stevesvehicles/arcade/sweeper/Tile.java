...
public class Tile {
	...
	public static enum TileOpenResult {
		OK,
		BLOB,
		FAILED,
		DEAD
	...
	public TileOpenResult open() {
		if (state != TILE_STATE.OPENED && state != TILE_STATE.FLAGGED) {			
			state = TILE_STATE.OPENED;
			if (nearbyCreepers == 0) {
				game.emptyLeft--;
				return TileOpenResult.BLOB;
			}else if (isCreeper()) {
				return TileOpenResult.DEAD;
			}else{
				game.emptyLeft--;
				return TileOpenResult.OK;
			}
		}else{
			return TileOpenResult.FAILED;
		}		
	}
