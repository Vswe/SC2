...
public class ArcadeMonopoly extends ArcadeGame {
	...
	public ArcadeMonopoly(ModuleArcade module) {
		...
		pieces.add(new Piece(this, 1, Piece.ControlledBy.COMPUTER));
		pieces.add(new Piece(this, 2, Piece.ControlledBy.COMPUTER));
		pieces.add(new Piece(this, 3, Piece.ControlledBy.COMPUTER));
		pieces.add(new Piece(this, 4, Piece.ControlledBy.COMPUTER));
		...
		if (getCurrentPiece().getController() == Piece.ControlledBy.COMPUTER) {
			roll.onClick();
		}
	}
	
	private boolean useAI() {
		return getCurrentPiece().getController() == Piece.ControlledBy.COMPUTER;
	}
	...
	public void drawBackground(GuiVehicle gui, int x, int y) {
		...
		for (int i =  0; i < pieces.size() ; i++) {
			...
			for (int j = 0; j < 3 ; j++) {
				...
				switch (j) {
					case 0:
						v = piece.getController() == Piece.ControlledBy.PLAYER ? 0 : piece.getController() == Piece.ControlledBy.COMPUTER ? 1 : 2;
						break;
					case 1:
	...
	private void drawPropertyOnBoardWithPositionRotationAndScale(GuiVehicle gui, Place place, int id,  boolean zoom, boolean hover, int x, int y, int r, float s) {
		...
		EnumSet<Place.PlaceState> states = EnumSet.noneOf(Place.PlaceState.class);
		if (zoom) {
			states.add(Place.PlaceState.ZOOMED);
		}else if(hover) {
			states.add(Place.PlaceState.HOVER);
		}
		
		if (selectedPlace == id) {
			states.add(Place.PlaceState.SELECTED);
		}
		...
		if (place instanceof Property) {
			...
			if (property.hasOwner() && property.getOwner().showProperties()) {
				states.add(Place.PlaceState.MARKED);
			}
		}
        ...
        for (Piece piece : pieces) {
            if (!piece.isBankrupt() && piece.getPosition() == id) {
                total[place.getPieceAreaForPiece(piece)]++;
            }
        }
        ...
        for (Piece piece : pieces) {
            if (!piece.isBankrupt() && piece.getPosition() == id) {
                loadTexture(gui, 1);
                int area = place.getPieceAreaForPiece(piece);
                place.drawPiece(gui, piece, total[area], pos[area]++, area, states);
            }
        }
